...
HAL_StatusTypeDef UartSendByte(UART_HandleTypeDef *huart, uint8_t ucByte)
{
    if (__HAL_UART_GET_FLAG(huart, UART_FLAG_TXE))
    {
        ...
        huart->Instance->DR = (ucByte & (uint8_t)0x00FF);
			#else
				huart->DR = (ucByte & (uint8_t)0x00FF);
			#endif
        return HAL_OK;
    }
    else
...
* 说    明: 无
**/
void UartSendBuffer(UART_HandleTypeDef *huart, uint8_t *ucBuffer, uint16_t usLen)
{
    int i = 0;
...
HAL_StatusTypeDef UartReadByte(UART_HandleTypeDef *huart, uint8_t *ucByte)
{
    if (__HAL_UART_GET_FLAG(huart, UART_FLAG_RXNE))
    {
        ...
        *ucByte = (uint8_t)(huart->Instance->DR & (uint8_t)0x00FF);
			#else
				*ucByte = (uint8_t)(huart->DR & (uint8_t)0x00FF);
			#endif
...
* 说    明: 用在串口初始化之后，修改波特率
**/
extern void setBaudRate_Usart1(uint32_t buad);
                      ...
                      FuncPtrU8_t ReciveNewFunc)
{
    ...
    pFIFO_Uart->ReciveNewFunc = ReciveNewFunc;
...
* 说    明: 无
**/
void FIFO_UartSetReciveNewFunc(UART_FIFO_Typedef_t *pFIFO_Uart, FuncPtrU8_t ReciveNewFunc)
{
    pFIFO_Uart->ReciveNewFunc = ReciveNewFunc;
}
...
/**
  ...
  * 说    明: 无
  */
void FIFO_UartSendBuffer(UART_FIFO_Typedef_t *pFIFO_Uart, uint8_t *ucBuffer, uint16_t usLen)
{
    uint16_t i;
...
* 说    明: 无
**/
void FIFO_UartSendByte(UART_FIFO_Typedef_t *pFIFO_Uart, uint8_t ucByte)
{
    FIFO_UartSendBuffer(pFIFO_Uart, &ucByte, 1);
}
...
void FIFO_UartIRQ(UART_FIFO_Typedef_t *pFIFO_Uart)
{
    ...
    if (__HAL_UART_GET_IT_SOURCE(pFIFO_Uart->huart, UART_IT_RXNE))
    {
        ...
        uint8_t ch;
        if (UartReadByte(pFIFO_Uart->huart, &ch) == HAL_OK)
        {
            ...
            if (pFIFO_Uart->ReciveNewFunc)
            {
                pFIFO_Uart->ReciveNewFunc(ch);
            }
        }
    }
    /* 处理发送缓冲区空中断 */
    if (__HAL_UART_GET_IT_SOURCE(pFIFO_Uart->huart, UART_IT_TXE))
    {
        if (pFIFO_Uart->usTxCount == 0)
        {
            ...
            /* 使能数据发送完毕中断 */
            __HAL_UART_ENABLE_IT(pFIFO_Uart->huart, UART_IT_TC);
        }
        else
        {
    ...
    /* 数据bit位全部发送完毕的中断 */
    else if (__HAL_UART_GET_IT_SOURCE(pFIFO_Uart->huart, UART_IT_TC))
    {
        if (pFIFO_Uart->usTxCount == 0)
        {
            /* 如果发送FIFO的数据全部发送完毕，禁止数据发送完毕中断 */
            __HAL_UART_DISABLE_IT(pFIFO_Uart->huart, UART_IT_TC);
